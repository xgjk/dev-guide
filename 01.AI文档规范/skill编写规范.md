...
| 附录 A-D | 4 个固定文件的完整内容 + 写入路径 | 定义"原样复制什么" |
| 附录 E | 快速参考卡片 | 一览表 |
| 附录 F | 完整迷你示例（1 模块 1 接口） | 定义"最终产物长什么样" |
...
└── scripts/
    ├── common/
    │   └── toon_encoder.py              # TOON 编码器（固定代码，见附录 D）
    └── <module>/
        ├── README.md                    # 脚本清单
...
| `scripts/` | **执行层** | 定义每个接口"怎么调"：Python 脚本调 API + TOON 编码输出 | **动态**，根据业务生成 |
...
SKILL.md（索引层）
  ...
  └── 路由 → openapi/<module>/api-index.md（文档层 — 模块入口）
                ...
                └── 列出 → openapi/<module>/<endpoint>.md（文档层 — 接口详情）
                              ...
                              │                        │
                              │                        ├── 引入 → scripts/common/toon_encoder.py（共享依赖）
...
| `scripts/common/toon_encoder.py` | **固定** | 附录 D，原样复制 | 共享依赖，所有脚本引入 |
...
6. **鉴权一致**：脚本内部同样遵循 `common/auth.md` 的鉴权规则。
...
| 脚本 | 对应接口 | 用途 |
|---|---|---|
| `<endpoint>.py` | `POST /path/to/endpoint` | 调用接口，输出 TOON 编码结果 |
...
**关键约束**：
- 必须引入 `toon_encoder`：`from toon_encoder import encode as toon_encode`
- 必须在输出前经过 `toon_encode()`：`print(toon_encode(result))`
...
```
总文件数 = 4（固定文件）+ 1（SKILL.md）+ M × 3（每个模块的 api-index.md + examples/README.md + scripts/README.md）+ N × 2（每个 API 的 endpoint.md + endpoint.py）

其中：M = 模块数量，N = 所有模块的 API 总数
```

**示例**：1 个模块、3 个 API → `4 + 1 + 1×3 + 3×2 = 14 个文件`
...
mkdir -p <skill-name>/scripts/common
mkdir -p <skill-name>/scripts/<module>
```

#### 1b. 写入 4 个固定文件

从本文档附录中**原样复制**以下 4 个文件，**禁止做任何修改**：
...
| 4 | 附录 D | `<skill-name>/scripts/common/toon_encoder.py` |

写入后确认 4 个文件全部存在，缺一不可。

#### ✅ 步骤 1 自检关卡（必须全部通过才能进入步骤 2）

```
□ 目录结构是否已全部创建？（common/, openapi/common/, openapi/<module>/, examples/<module>/, scripts/common/, scripts/<module>/）
□ common/auth.md 是否存在且内容完整？
□ common/conventions.md 是否存在且内容完整？
□ openapi/common/appkey.md 是否存在且内容完整？
□ scripts/common/toon_encoder.py 是否存在且内容完整？
□ 以上 4 个固定文件是否从附录 A-D 原样复制，未做任何修改？
```
...
for 第 i 个 API (i = 1 到 N):
  ...
  │    按 §2.6 标准模式：XG_USER_TOKEN 环境变量 → API_URL 硬写 → 构造请求 → 调API → toon_encode输出
  │    入参字段必须与 ① 的参数表完全一致
  │
  └─ ③ 补充触发场景到  examples/<module>/README.md
       触发条件、标准流程、用户实际会说的话

  → 第 i 个 API 的迷你自检（3 项全过才进入下一个）：
     □ endpoint.md 的 URL 与 endpoint.py 的 API_URL 是否完全一致？
     □ endpoint.md 的参数表字段 与 endpoint.py 的请求 body 字段是否完全一致？
     □ endpoint.py 是否包含 toon_encoder 引入和 toon_encode() 输出？
...
□ 每个脚本是否都有 from toon_encoder import encode as toon_encode？
...
4. **脚本引入**：列出每个 `.py` 文件，确认含 `from toon_encoder import encode as toon_encode` 和 `toon_encode()` 调用
5. **占位符清除**：在所有文件中搜索 `<module>`、`<endpoint>`、`<skill-name>` 等占位符，列出搜索结果
6. **入参校验**：列出每个脚本的 JSON body 字段，与对应 endpoint.md 的参数表逐一比对

#### 5c. 第三轮：与参考示例结构比对

对照本文档**附录 F 的完整示例**，检查生成的 Skill 包是否具有相同的结构特征：

```
□ 目录层级是否与示例一致？（common/, openapi/common/, openapi/<module>/, examples/<module>/, scripts/common/, scripts/<module>/）
□ SKILL.md 是否包含示例中的所有必备节（宪章、概览、工作流、路由表、能力树）？
□ 每个 endpoint.md 是否包含示例中的所有必备节（作用、Headers、参数表、请求Schema、响应Schema、脚本映射）？
□ 每个 endpoint.py 是否遵循示例中的脚本结构（docstring → import → sys.path → API_URL → call_api → main → 守卫）？
...
| # | 常见错误 | 正确做法 |
|---|---|---|
| 1 | 修改了固定文件（auth.md / conventions.md / appkey.md / toon_encoder.py）的内容 | 从附录 A-D **原样复制**，一个字都不改 |
...
| 6 | 脚本缺少 `from toon_encoder import encode as toon_encode` 或忘记调用 `toon_encode()` | 每个业务脚本必须引入并使用 toon_encoder |
| 7 | SKILL.md 能力树与实际目录结构不一致 | 能力树必须最后更新，与实际文件一一对应 |
| 8 | 占位符（`<module>`、`<endpoint>`、`<skill-name>`）残留在最终产物中 | 全文搜索并替换为实际值 |
| 9 | 使用了绝对路径（如 `/home/user/skill/...`） | 所有路径必须使用相对路径 |
| 10 | 脚本中直接 `print(result)` 输出原始 JSON | 必须 `print(toon_encode(result))`，经过 TOON 编码后再输出 |
| 11 | 向用户询问 token / 鉴权 / 登录相关问题 | 鉴权是固定文件自动处理的，禁止问用户 |
| 12 | 跳过了步骤间的自检关卡 | 每个步骤完成后必须逐项自检，全部通过才进入下一步 |
| 13 | 反思时只写"全部通过"没有列出证据 | 第二轮交叉验证必须输出文件清单、绑定对照表等具体证据 |
| 14 | 不知道最终产物应该长什么样 | 对照附录 F 的完整示例，确保结构和格式一致 |
...
- [ ] `scripts/common/toon_encoder.py` 存在
...
- [ ] 每个脚本包含：toon_encoder 引入、API 调用、toon_encode 输出
...
> 以下检查仅针对 `scripts/<module>/` 下的业务脚本。`scripts/common/` 下的共享依赖和 `openapi/common/` 下的固定参考文档不在此检查范围内。

- [ ] 每个业务接口都有对应 `.py` 脚本（不允许"暂无脚本"；`openapi/common/appkey.md` 除外，它是固定参考文档）
- [ ] 每个业务脚本都引入了 `from toon_encoder import encode as toon_encode`
- [ ] 每个业务脚本的 API 响应都经过 `toon_encode()` 后再 `print`
...
- [ ] `scripts/common/toon_encoder.py` 内容完整未被修改
...
## 附录 D：scripts/common/toon_encoder.py

> **写入路径**：`<skill-name>/scripts/common/toon_encoder.py`
> **性质**：固定代码，原样复制，禁止修改。禁止任何重构、精简、变量重命名、增删注释。
...
| `toon_encoder.py` | 附录 D | `scripts/common/toon_encoder.py` | **固定**，原样复制 |
...
>
> 文件计数：4（固定）+ 1（SKILL.md）+ 1×3（模块索引）+ 1×2（接口文件）= **10 个文件**
...
6. **执行对应脚本**：调用 `scripts/forecast/get-current.py` 执行接口调用，获取 TOON 编码后的结果。**所有接口调用必须通过脚本执行，不允许跳过脚本直接调用 API。**

脚本使用规则（强制）：
1. **每个接口必须有对应脚本**：每个 `openapi/forecast/<endpoint>.md` 都必须有对应的 `scripts/forecast/<endpoint>.py`，不允许"暂无脚本"。
2. **TOON 编码输出**：所有脚本调用 API 后，响应 JSON **必须经过 `scripts/common/toon_encoder.py` 编码后再输出**，不允许直接输出原始 JSON。
3. **脚本可独立执行**：所有 `scripts/` 下的脚本均可脱离 AI Agent 直接在命令行运行。
4. **先读文档再执行**：执行脚本前，**必须先阅读对应模块的 `openapi/forecast/api-index.md`**。
5. **入参来源**：脚本的所有入参定义与字段说明以 `openapi/` 文档为准，脚本仅负责编排调用流程。
6. **鉴权一致**：脚本内部同样遵循 `common/auth.md` 的鉴权规则。
...
### F.3 openapi/forecast/api-index.md
...
### F.4 openapi/forecast/get-current.md
...
### F.5 examples/forecast/README.md
...
### F.6 scripts/forecast/README.md
...
| 脚本 | 对应接口 | 用途 |
|---|---|---|
| `get-current.py` | `GET /openapi/weather/current` | 查询当前天气，输出 TOON 编码结果 |
...
### F.7 scripts/forecast/get-current.py
...
> **注意**：以上示例中，`common/auth.md`、`common/conventions.md`、`openapi/common/appkey.md`、`scripts/common/toon_encoder.py` 这 4 个固定文件的内容从附录 A-D 原样复制，此处不重复展示。
