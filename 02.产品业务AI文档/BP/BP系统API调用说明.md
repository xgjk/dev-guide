...
**配套文档**：[《BP 目标管理 Open API 接口定义》](./BP系统API说明.md)（数据模型 **一、1.4**、**二、2.1～2.37** 接口契约、**三、3.x** 公共类型、**四、错误码**）。
...
| 1.16 | 2026-04-07 | 新增任务承接全链路接口：2.24 查指派树、2.25 创承接目标、2.26 对齐任务、2.27 更新任务 | 刘会芳 |
| 1.17 | 2026-04-08 | 新增版本管理链：2.28 查历史列表、2.29 查快照详情、2.30 版本回退；公共类型新增 **三、3.20 TaskVersionSnapshotVO** | 刘会芳 |
| 1.18 | 2026-04-10 | 新增 2.31 按月份查询月报列表（`listMonthlyReports`，仅返回类型+汇报ID）、2.32 查询月度评价 Markdown（`getMonthlyEvaluation`，formJson 翻译为 AI 可读 Markdown） | 曾文哲 |
| 1.19 | 2026-04-13 | Group 对象新增 `deptId` 字段（`type=org` 时返回关联部门 ID）；批量获取部门信息接口移至基础服务（用户服务） | 曾文哲 |
| 1.20 | 2026-04-15 | 新增 2.33 保存AI生成月报内容（`saveOpenClawReport`，写入 `bp_openclaw_task` 生成记录，不发送工作汇报） | 曾文哲 |
| 1.21 | 2026-04-22 | 新增 2.34 获取汇报推进情况 Markdown（`getReportProgressMarkdown`，按任务ID+月份返回汇报AI摘要与回复） | 曾文哲 |
| 1.22 | 2026-04-22 | 2.22 保存月度汇报的 `reportRecordId` 改为非必填；未发送工作汇报时可不传 | 曾文哲 |
| 1.23 | 2026-05-15 | 历史路径 `/bp/task/v2/getGoalAndKeyResult` 的 `keyResults[]` 响应新增 `keyResultLampReason`，供单目标月报 Skill 采集 KR 灯色判断；不纳入公共 `KeyResultFullVO` | 曾文哲 |
| 1.24 | 2026-05-15 | 目标、关键成果、关键举措详情响应新增 `startTime`、`closeTime`，用于按实际启动/关闭时间判断任务生命周期 | 曾文哲 |

> **关于小节号**：版本 **1.10 及以前**的变更摘要若仍写「5.x」「第六章」或「3.x/4.x 接口与类型」，指当时**单文件**或旧版双文档编号；与当前《接口定义》对照时，接口为 **二、2.1～2.37**，公共类型为 **三、3.1～3.20**，字段规格为 **一、1.4.x**。
...
24. **getUndertakeTaskTree** — 查询待承接任务树（含指派路径与统计）
25. **addGoal** — 创建承接目标并同时关联指派任务（写操作）
26. **alignTask** — 显式建立任务对齐关系（写操作）
27. **updateTask** — 通用任务更新（写操作，包含名称、时间、权重及5类参与人/部门）
28. **getHistoryPage** — 查询任务版本历史列表
29. **getHistoryDetail** — 获取指定快照版本的全量详情
30. **rollback** — 将任务回退到指定的历史快照版本（写操作）
31. **listMonthlyReports** — 按月份查询月报列表（仅返回报告类型描述+汇报记录ID，AI通过工作协同接口获取正文）
32. **getMonthlyEvaluation** — 查询月度评价 Markdown（formJson 已翻译为 AI 可直接阅读的 Markdown，含目标名称、评分维度中文、评语）
33. **saveOpenClawReport** — 保存 AI 生成月报内容到生成记录（写入 `bp_openclaw_task`，不发送工作汇报，供人工审阅）
34. **getReportProgressMarkdown** — 获取汇报推进情况 Markdown（按任务ID+月份返回汇报的AI结构化摘要与回复，供模型消费）
...
4. **《接口定义》二、2.1～2.37**：具体方法的参数表、示例与 `data` 类型声明。
5. **《接口定义》三、3.1～3.20**：嵌套 VO 的补充说明（与 **一、1.4** 互为引用）。
6. **《接口定义》四、错误码**：失败时 `resultCode` / `resultMsg`。
7. 需要**调用链**时读本文 **七、关键业务流程**；需要**省 Token / 通读策略**时读本文 **六、推荐拉取路径**。

**写操作（有副作用，默认只读场景勿调用）**：**2.9** 发送延期提醒、**2.14** 新增关键成果、**2.15** 新增关键举措、**2.22** 保存月度汇报、**2.25** 创建承接目标、**2.26** 对齐任务、**2.27** 更新任务、**2.30** 版本回退。
...
| 2.24 | getUndertakeTaskTree | POST | `/bp/task/v2/getUndertakeTaskTree` | 读 | 指派任务树（带统计） |
| 2.25 | addGoal | POST | `/bp/task/v2/addGoal` | **写** | 创建并对齐目标 |
| 2.26 | alignTask | POST | `/bp/task/v2/alignTask` | **写** | 手动建立任务对齐 |
| 2.27 | updateTask | POST | `/bp/task/v2/updateTask` | **写** | 通用内容/人员更新 |
| 2.28 | getHistoryPage | GET | `/bp/task/v2/getHistoryPage` | 读 | 查询任务版本历史列表 |
| 2.29 | getHistoryDetail | GET | `/bp/task/v2/getHistoryDetail` | 读 | 获取快照版本的全量详情 |
| 2.30 | rollback | POST | `/bp/task/v2/rollback` | **写** | 任务版本回退 |
| 2.31 | listMonthlyReports | GET | `/bp/monthly/report/listByMonth` | 读 | 按月份查询月报列表（类型+汇报ID） |
| 2.32 | getMonthlyEvaluation | GET | `/bp/monthly/evaluation/query` | 读 | 月度评价 Markdown（formJson 翻译后） |
| 2.33 | saveOpenClawReport | POST | `/bp/openclaw/saveReport` | **写** | 保存AI生成月报到生成记录 |
| 2.34 | getReportProgressMarkdown | GET | `/bp/task/reportProgress` | 读 | 汇报推进情况 Markdown（按任务+月份） |
| 2.35 | saveTaskMonthlyReading | POST | `/bp/task/monthlyReading/save` | **写** | 保存目标月报信息 |
| 2.36 | getTaskMonthlyReading | GET | `/bp/task/monthlyReading/get` | 读 | 查询目标月报信息 |
| 2.37 | getMonthlyReportRevisionContext | POST | `/bp/monthly/report/revision/context` | 读 | 月报修订上下文（reportId + 编码 → group/month/任务ID映射） |
...
> 需求：查看上级指派的任务，并创建自己的目标进行承接。

1. **2.24**（`POST /bp/task/v2/getUndertakeTaskTree`）→ 查看 `pendingCount` > 0 的分组，获取 `tasks[].id`。可通过 `fullPath` 查看任务所属的上级目标链路。
2. **2.25**（`POST /bp/task/v2/addGoal`）→ 填入 `upwardTaskIdList` (第1步中的任务ID)，完成创建并对齐。
3. **2.27**（`POST /bp/task/v2/updateTask`）→ 后续按需调整名称、截止时间或参与人（如添加监督人/协办人）。

### 场景九：基于快照的任务变更审计与回退流程

> 需求：当 AI 执行了错误的修改，或用户需要找回历史版本时。

1. **2.28**（`GET /bp/task/v2/getHistoryPage?taskId={id}`）→ 获取该任务的所有版本流水。
2. **2.29**（`GET /bp/task/v2/getHistoryDetail?id={snapshotId}`）→ 获取特定版本的全量快照 JSON。
3. **内容比对**：调用方将 `snapshotData` 与当前任务数据进行比对，渲染出“变更差异单”。
4. **执行回滚**：**2.30**（`POST /bp/task/v2/rollback`）→ 传入 `snapshotId`，任务状态一键恢复至历史时刻。

### 场景十：获取上月汇报与评价（月报生成参考上月基线）

> 需求：生成本月月报时，参考上个月的汇报内容和评价反馈，做纵向对比分析。

1. **2.31**（`GET /bp/monthly/report/listByMonth?groupId={id}&reportMonth={上月YYYY-MM}`）→ 获取上月所有类型月报的 `reportRecordId` 和 `reportTypeDesc`。
2. 对每个 `reportRecordId`，调用工作协同接口 `/work-report/report/info?reportId={reportRecordId}` 获取报告正文和回复。
3. **2.32**（`GET /bp/monthly/evaluation/query?groupId={id}&evaluationMonth={上月YYYY-MM}`）→ 获取上月自评和上级评价的 Markdown（已翻译，含目标名称、各维度评分、评语）。
4. 将上月汇报正文 + 评价 Markdown 作为本月月报生成的参考输入。

### 场景十一：部门月报数据采集

> 需求：为组织节点（部门）生成月度自查报告，需采集部门自身 BP 数据及下对齐承接情况。

1. **2.1** → **2.2**（`GET /bp/group/list?periodId={id}`）→ 遍历 `type=org` 节点，获取部门 `groupId` 和 `deptId`。
2. 调用基础服务（用户服务）`POST /cwork-user/department/listByIds` → 传入 `deptId` 列表，获取部门负责人 `deptLeaderId`（用于报告发送）。详见 [《基础服务 API 接口文档》](../基础服务/基础服务API说明.md)。
3. **2.4**（`GET /bp/task/v2/getSimpleTree?groupId={部门groupId}`）→ 获取部门 BP 任务树。
4. 对每个目标调 **2.5**（`getGoalDetail`）→ 获取 `downTaskList`（下对齐任务列表），记录对齐到本部门的外部目标 ID。
5. 对下对齐目标调 **2.5** 获取目标级信息（仅目标层，不递归 KR/Action）。
6. **2.8**（`listTaskReports`）→ 分别获取部门自身任务和下对齐目标的当月汇报。
7. 汇总数据后生成部门自查报告，通过 **2.9** 发送给部门负责人。
