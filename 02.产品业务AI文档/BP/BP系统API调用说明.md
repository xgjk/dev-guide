...
**配套文档**：[《BP 目标管理 Open API 接口定义》](./BP系统API说明.md)（数据模型 **一、1.4**、**二、2.1～2.20** 接口契约、**三、3.x** 公共类型、**四、错误码**）。
...
> **关于小节号**：版本 **1.10 及以前**的变更摘要若仍写「5.x」「第六章」或「3.x/4.x 接口与类型」，指当时**单文件**或旧版双文档编号；与当前《接口定义》对照时，接口为 **二、2.1～2.20**，公共类型为 **三、3.1～3.16**，字段规格为 **一、1.4.x**。
...
8. **listTaskReports** — 按任务 ID 分页查汇报（任务id指目标或者关键成果或者举措id,只获取本任务关联的汇报信息，不含下级任务。汇报主要返回是正文，不含附件和回复）
9. **发送 AI 延期提醒汇报** — 写操作
