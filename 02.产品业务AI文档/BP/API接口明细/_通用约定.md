...
> 本文件为 BP 目标管理 Open API 的**通用约定与业务流程**文档。AI 在加载子文档时应同时（或先于）加载此文件，无需再引用 `../BP系统API调用说明.md`。
...
> **关于小节号**：版本 **1.10 及以前**的变更摘要若仍写「5.x」「第六章」或「3.x/4.x 接口与类型」，指当时**单文件**或旧版双文档编号；与当前子文档对照时，接口编号为 **2.1～2.37**。

---

## 一、概述

BP（Business Plan）目标管理系统对外开放 HTTP 接口后，调用方可实现下列能力（与各子文档 **二、2.x** 一一对应）：
...
| Header 约定 | Header 名称固定为 `appKey` |
| 兼容说明 | 某些场景下，header 有 `access-token` 但没有 `appKey`，系统也能兼容 |
| 鉴权机制 | 仅应用级鉴权 |
...
### 3.7 给 AI / Agent 的阅读顺序

1. **本文「三、通用说明」**：`Result<T>`、`appKey`、环境与 URL 形态。
2. **本文「六、接口清单」**：按任务快速定位小节号与路径。
3. **《总纲》一、1.4**：所有响应字段的**唯一权威**（含 `groupId` 是否在详情中出现等）。
4. **各子文档二、2.1～2.37**：具体方法的参数表、示例与 `data` 类型声明。
5. **各子文档公共数据结构部分**：嵌套 VO 的补充说明（与 **一、1.4** 互为引用）。
6. **本文「八、错误码说明」**：失败时 `resultCode` / `resultMsg`。
7. 需要**调用链**时读本文 **五、关键业务流程**；需要**省 Token / 通读策略**时读本文「四、推荐拉取路径」。
...
以下说明接口之间的**推荐调用顺序**与参数传递；契约细节见各子文档 **二、2.x**。
...
4. **2.4 与 2.5～2.7**：`TaskTreeVO` 为简要导航；详情字段以《总纲》**一、1.4.3～1.4.5** Full 列与 **三、3.2** 为准，勿按已废弃的 `BaseTaskVO` 继承理解。
