...
单目标月报 Skill 当前仍通过历史路径 `/bp/task/v2/getGoalAndKeyResult?id={goalId}` 采集目标结构。该历史路径的内嵌 `keyResults[]` 会额外返回 `keyResultLampReason`，用于关键成果灯色判断；规范路径 `/bp/goal/{goalId}/detail` 的 `KeyResultFullVO` 不包含该字段。
    ...
    "keyResults": [
      {
        ...
        "name": "客户拜访量达到50家",
        "measureStandard": "拜访记录数 >= 50",
        "keyResultLampReason": "根据关键成果衡量标准、目标完成情况和下级关键举措综合判断，本关键成果当前为绿灯。",
        "actions": []
      }
    ]
  }
