...
> [!NOTE]
> **个人知识库特殊性说明**：
> 个人知识库本质上就是一个**拥有固定唯一归属人、无复杂权限体系的特殊 projectId**。获取到该 `projectId` 后，您可以将其传入任何带有 `projectId` 参数的**通用空间接口**（如 `4.16 saveFileByParentId` 等），从而实现向个人知识库执行高级通用操作的目的。详情可参阅《总纲》的 2.4 节。
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `projectId` | Long | 否 | 空间 ID；不传时服务端会**默认使用个人知识库**（行为等价于先调用 **4.8 getProjectId** 获取个人空间 `projectId`）。**前置条件**：需能从请求头获取 `employeeId/corpId`；若 `corpId` 缺失或该企业/场景不开放个人空间，则接口会返回错误（不会退化为跨空间查询）。 |
