...
| 版本号 | v2 |
| 接口类型 | 查询 |
| 推荐调用场景 | UI 分页展示文件内容；对纯文本类文件可作为"全量正文"获取入口（与 4.4 等价） |
...
| `md` / `txt` / `html` / `htm` / `xml` / `notex` / `xls` / `xlsx` | **无分页** | **被忽略** | **全量正文**（与 4.4 `getFullFileContent` 同源、内容一致） |
...
| 场景 | 返回 |
| :--- | :--- |
| `fileId` 不存在 / 已删除 / `resourceId` 未生成 | `data: null`（后端打 warn 日志） |
| 远端 file-convert 抽取异常 | `data: null`（后端记录 error 日志） |
| 上述两类不再返回 `data: ""` 误导调用方 | — |
...
**与 4.4 `getFullFileContent` 的选择建议**

- 想要"按页 / 流式 / UI 阅读体验" → 用 4.3，并对有分页类型传入 `pageNumber`。
- 想要"一次拿全文喂给 AI / 拼接 prompt" → 用 4.4。
- 对**纯文本类**文件（md/txt/html 等），4.3 与 4.4 返回内容**等价**；首选 4.4 表达"我要全量"语义更清晰。
