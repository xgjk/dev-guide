...
**与 4.3 `getFileContent` 的关系（v2 起）**

- 4.4 永远返回**全量正文**，AI / 拼接 prompt 首选；
- 4.3 优先用于"UI 流式分页阅读"，对**有分页类型**（pdf/pptx/docx 等）按 `pageNumber` 取该页文本；
- 对**无分页类型**（md / txt / html / htm / xml / notex / xls / xlsx），4.3 与 4.4 内容**等价**——4.3 会忽略 `pageNumber` 直接返回全量正文。详见 [04-UI终端预览与阅读.md](04-UI终端预览与阅读.md#43-获取ui阅读模式根据文件id和页码分页获取内容)。
