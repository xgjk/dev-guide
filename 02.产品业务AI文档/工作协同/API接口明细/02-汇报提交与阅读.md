...
> **业务流程说明**：关于如何按层级/分组指定接收人以及如何上传并关联附件，请阅读 **[场景一：发送汇报 或 新增/更新草稿 通用参数说明](_通用约定.md#场景一发送汇报-或-新增更新草稿-通用参数说明)**。
> 
> **业务单元智能流转指引**：若用户要求按特定的业务组/方案发送汇报（涉及推断或使用 `businessUnitId`），大模型请务必优先阅读并遵循 **[场景三：用户想要通过 业务单元 进行汇报](_通用约定.md#场景三用户想要通过-业务单元-进行汇报)** 中的匹配拦截规则。
> 
...
| `formTemplateId`  | Long                    | 否   | 表单模板 ID                                                                               |
| `formDataParam`   | FormInstanceRequest     | 否   | 表单数据参数；结构见 **6.32 FormInstanceRequest**                                            |
...
### 6.32 FormInstanceRequest

表单实例请求体，用于在提交汇报时附带表单数据。

| 字段名          | 类型                         | 说明                                                    |
| --------------- | ---------------------------- | ------------------------------------------------------- |
| `dataList`      | List\<FormDataRequest>       | 表单数据列表（见下表）                                  |

**FormDataRequest**：

| 字段名          | 类型                         | 说明                                                    |
| --------------- | ---------------------------- | ------------------------------------------------------- |
| `propertyName`  | String                       | 属性标题                                                |
| `dataValue`     | Object                       | 属性值                                                  |

> [!NOTE]
> **表单属性智能匹配规则说明**：
> 1. 为了简化调用，调用方**无需**传入难以获取的 `propertyId`（属性唯一 ID），只需在 `propertyName` 中传入对应的**属性标题**（表单设计器中配置的字段名称，例如：“项目进度”、“完成比例”等），并在 `dataValue` 中传入具体的**属性值**即可。
> 2. **后端自动匹配与校验机制**：
>    - 后端服务在接收到请求后，会首先加载 `formTemplateId` 对应的全部表单属性配置。
>    - 系统将自动遍历 `dataList`，通过比较 `propertyName` 与模板配置中的属性名称来自动匹配并注入正确的 `propertyId`。
>    - **异常校验**：若 `dataList` 中的某个 `propertyName` 在当前表单模板中不存在（例如拼写错误或不存在该属性），后端将直接抛出业务异常：`"表单参数未匹配表单模板信息，检查参数属性ID/名称是否正确"`。
> 3. **最佳实践**：请在调用前务必确认表单模板中的属性标题与传入的 `propertyName` 字面完全一致（大小写、空格需一致）。
